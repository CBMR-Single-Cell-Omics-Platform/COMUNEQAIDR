...
#' @export
infer_and_check_config <- function(config, sheet) {
...
hto_features <- function(sheet) {
  all_samples <- sheet[, c("sample", "hash_index", "hto")] #nucleus_isolation_id?
...
prepare_comuneqaid <- function(comuneqaid_id,
  ...
  config <- infer_and_check_config(config, sheet)
  ...
  # Create pool table
  pool_table <- make_pool_table(sheet, split_by = c(
    "nucleus_isolation_id",
    "i7_index", "lane"
  ))
  config[["pool_table"]] <- pool_table
